...
function updateCursor(event, originalValue, originalPosition) {
  ...
  var inputType = event.inputType || 'insertText'; // get some information about the cursor based on the original value

  var isInsertEvent = ['insertText', 'insertFromPaste'].includes(inputType);
...
var tokenDefinitions = src_tokens;
/**
...
/* harmony default export */ var directive = ({
  ...
  inserted: function inserted(el) {
    el = getInputElement(el);
    var config = el[directive_CONFIG_KEY]; // prefer adding event listener to parent element to avoid Firefox bug which does not
    // execute `useCapture: true` event handlers before non-capturing event handlers

    var handlerOwner = el.parentElement || el; // use anonymous event handler to avoid inadvertently removing masking for all inputs within a container
    ...
    config.cleanup = function () {
      return handlerOwner.removeEventListener('input', handler, true);
    };
  },
